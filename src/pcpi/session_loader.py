...
def __validate_twistlock_credentials(
    name, _url, uname, passwd, verify, proxies
) -> bool:
    """
    This function creates a session with the supplied credentials to test
    if the user successfully entered valid credentials.
    """
    py_logger.debug("validating twistlock credentials entry")
    ...
    try:
        ...
        res = requests.request(
            "POST", url, headers=headers, json=payload, verify=verify, proxies=proxies
        )
        print(res.status_code)
        py_logger.debug(res.status_code)
        ...
        print()
        __c_print(
            "2) Please ensure you have entered a valid Prisma Cloud URL.", color="blue"
        )
        print()
        logging.info("Can't validate twistlock credentials")
        return False
...
def __validate_credentials(a_key, s_key, url, verify, proxies) -> bool:
    ...
    if the user successfully entered valid credentials.
    """
    py_logger.debug("_validate_credentials entry")
    headers = {"content-type": "application/json; charset=UTF-8"}

    payload = {"username": f"{a_key}", "password": f"{s_key}"}
...
def __universal_validate_credentials(name, url, tl_url, _id, secret, verify, proxies):
    py_logger.debug("universal validating Credentials entry")
    if verify.lower() == "true":
        verify = True
    else:
        verify = False
        if __validate_credentials(
            _id, secret, url, verify, proxies
        ) & __validate_twistlock_credentials(
            name, tl_url, _id, secret, verify, proxies
        ):
            py_logger.debug("Credentials validated successfully")
            return True
...
def __get_config():
    py_logger.debug("Capturing config from input")
    __c_print(
        "Enter Prisma URL. (SaaS EX: https://app.ca.prismacloud.io, On-Prem EX: https://yourdomain.com):",
        color="blue",
    )
    url = input()
    print()
    ...
    if new_url != url:
        ...
        print()
    __c_print(
        "Enter Twistlock Api URL. (SaaS EX: https://us-east1.twistlock.com/us-2-158320372, On-Prem EX: https://yourdomain.com):",
        color="blue",
    )
    tl_url = input()
    print()
    # new_tl_url = __validate_url(tl_url)
    new_tl_url = tl_url
    if new_tl_url != tl_url:
        __c_print("Adjusted URL:", color="yellow")
        print(new_tl_url)
        print()
    ...
    __c_print(
        "Certificate verification: (True/False/<path_to_.pem> file)", color="blue"
    )
    ...
    __c_print(
        "Proxy section. HTTP first, HTTPS second. Leave blank to not use a proxy.",
        color="yellow",
    )
    ...
    if verify == "":
        ...
        pass
    py_logger.debug("Captured values / verify equals %s", verify)
    return name, _id, secret, new_url, new_tl_url, verify, proxies, project_flag


def __build_config_json(name, _id, secret, url, tl_url, verify, proxies, project_flag):
    session_dict = {
        "name": name,
        "url": url,
        "tl_url": tl_url,
...
def __get_config_from_user(num_tenants, min_tenants):
    ...
    if num_tenants != -1 and min_tenants == -1:
        for _ in range(num_tenants):
            ...
            while not valid:
                py_logger.debug(
                    "Getting config from user with num of tenants"
                )  # DEUG REMOVE ME
                __c_print("Enter Prisma Cloud Credentials", color="blue")
                print()
                name, _id, secret, url, tl_url, verify, proxies, project_flag = (
                    __get_config()
                )
                py_logger.debug(
                    "__get_config finished and assigned values succcessfully"
                )

                valid = __universal_validate_credentials(
                    name, url, tl_url, _id, secret, verify, proxies
                )
                py_logger.debug(
                    "after __universal_validate_credentials, valid value is %s", valid
                )
                if not valid:
        ...
        while True:
            ...
            while not valid:
                ...
                py_logger.debug("Credentials received")

                valid = __universal_validate_credentials(
                    name, url, tl_url, _id, secret, verify, proxies
                )
                py_logger.debug("Valid equals %s", valid)
                if not valid:
                    __c_print("FAILED", end=" ", color="red")
                    print("Invalid credentials. Please re-enter your credentials")
                    print()
        ...
        while True:
            ...
            while not valid:
                ...
                if valid == False:
                    ...
                    py_logger.debug("Appending credentials")
                    credentials.append(
                        __build_config_json(
                            name,
                            _id,
...
def load_config(file_path="", num_tenants=-1, min_tenants=-1, logger=py_logger):
    ...
    if not os.path.exists(config_path):
        py_logger.debug(
            ("Getting credentials from user as credentials don't exist on file")
        )
        config = __get_config_from_user(num_tenants, min_tenants)
        py_logger.debug("config %s", config)
    ...
    with open(config_path, "r") as infile:
        try:
            ...
            py_logger.error("Failed to load credentials file. Exiting...")
            py_logger.debug("Error %d", e)
...
def load_config_env(
    ...
    project_flag_name="PRISMA_PCPI_PROJECT_FLAG",
    logger=py_logger,
):
    error_exit = False
    ...
    try:
        ...
        logger.warning(
            f"Missing '{prisma_name}' environment variable. Using default name."
        )
    ...
    if "prismacloud.io" in api or "prismacloud.cn" in api:
        return SaaSSessionManager(
            name, a_key, s_key, api, verify, proxies, logger=logger
        )
    else:
        return CWPSessionManager(
            name, api, a_key, s_key, verify, proxies, project_flag, logger=logger
        )

...
class CSPMSession(Session):
    ...
    def _api_login(self) -> object:
        ...
        payload = {"username": f"{self.a_key}", "password": f"{self.s_key}"}
    ...
    def _expired_login(self) -> None:
        ...
        self.logger.warning(
            "Update your custom credentials file or the default file in `~/.prismacloud`. Exiting..."
        )
    ...
    def _api_refresh(self) -> None:
        ...
        try:
            ...
            res = requests.request(
                "GET",
                self.api_url + "/auth_token/extend",
